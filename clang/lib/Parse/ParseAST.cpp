...
void clang::ParseAST(Sema &S, bool PrintStats, bool SkipFunctionBodies) {
	PROFILER_WATCH_CTX(CTX_ParseAST, "clang::ParseAST");
  ...
  ASTConsumer *Consumer = &S.getASTConsumer();
	...
	PROFILER_WATCH_ON_CTX(CTX_NewParser, "new Parser");
  std::unique_ptr<Parser> ParseOP(
      new Parser(S.getPreprocessor(), S, SkipFunctionBodies));
  Parser &P = *ParseOP.get();
	PROFILER_WATCH_OFF_CTX(CTX_NewParser);

	PROFILER_WATCH_ON_CTX(CTX_CrashRecovery, "Crash Recovery");
  ...
  llvm::CrashRecoveryContextCleanupRegistrar<Parser>
	...
	PROFILER_WATCH_OFF_CTX(CTX_CrashRecovery);

	{
		PROFILER_WATCH_CTX(CTX_EnterMainSourceFile, "EnterMainSourceFile");
		S.getPreprocessor().EnterMainSourceFile();
	}

	{
		PROFILER_WATCH_CTX(CTX_StartTranslationUnit, "StartTranslationUnit");
  ...
  if (HaveLexer) {
		PROFILER_WATCH_ON_CTX(CTX_InitParser, "InitParser");
    ...
    EnterExpressionEvaluationContext PotentiallyEvaluated(
		...
		PROFILER_WATCH_OFF_CTX(CTX_InitParser);

		PROFILER_WATCH_CTX(CTX_ParseTopLevel, "for ParseTopLevel");
    for (bool AtEOF = P.ParseFirstTopLevelDecl(ADecl, ImportState); !AtEOF;
      AtEOF = P.ParseTopLevelDecl(ADecl, ImportState)) {
      // If we got a null return and something *was* parsed, ignore it.  This
      // is due to a top-level semicolon, an action override, or a parse error
      // skipping something.
			PROFILER_WATCH_CTX(CTX_HandleTopLevelDecl, "HandleTopLevelDecl");
  ...
  // Process any TopLevelDecls generated by #pragma weak.
	{
		PROFILER_WATCH_CTX(CTX_HandleTopLevel, "HandleTopLevel");
		for (Decl *D : S.WeakTopLevelDecls())
			Consumer->HandleTopLevelDecl(DeclGroupRef(D));
	}

  {
		PROFILER_WATCH_CTX(CTX_HandleTranslationUnit, "HandleTranslationUnit");
	  Consumer->HandleTranslationUnit(S.getASTContext());
  }

  {
		PROFILER_WATCH_CTX(CTX_Finalize, "Finalize");
